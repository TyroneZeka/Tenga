...
public class S3ImageStorageService implements ImageStorageService {
  ...
  @Override
  public String upload(MultipartFile file, UUID listingId) {
    validateFile(file);

    String extension = getExtension(file.getContentType());
    String storageKey = "listings/%s/%s.%s".formatted(listingId, UUID.randomUUID(), extension);
    ...
    try {
      ...
      log.info("Image uploaded: bucket={}, key={}", bucket, storageKey);
      return storageKey;
    } catch (IOException e) {
  ...
  public void delete(String storageKey) {
    ...
    log.info("Image deleted: bucket={}, key={}", bucket, storageKey);
  }
  ...
  private void validateFile(MultipartFile file) {
    ...
    if (file.getSize() > MAX_FILE_SIZE) {
      throw new BusinessException("File size exceeds maximum of 5MB");
    }
